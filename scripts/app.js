...
document
  .getElementById("burguer-menu")
  .addEventListener("click", UpdateMenuIcon);
document
  .getElementById("shift-mode")
  .addEventListener("click", UpdatePresentationMode);
document
  .getElementById("favorite-url")
  .addEventListener("click", () => LoadSection(constant.FAVORITEHTMLURL));
document
  .getElementById("gifos-url")
  .addEventListener("click", () => LoadSection(constant.GIFOSHTMLURL));
...
async function UpdateMenuIcon() {
  ...
  if (shift.innerHTML === constant.DARK) {
    menu.src =
      currentIcon === "burger.svg"
        ? "./assets/close.svg"
        : "./assets/burger.svg";
  } else if (
    currentIcon === "close-modo-noct.svg" &&
    shift.innerHTML === constant.LIGHT
  ) {
    menu.src = "./assets/burger-modo-noct.svg";
  } else {
    menu.src =
      currentIcon === "close.svg"
        ? "./assets/burger-modo-noct.svg"
        : "./assets/close-modo-noct.svg";
  }

  if (
    (currentIcon === "burger.svg" || currentIcon === "burger-modo-noct.svg") &&
...
async function UpdatePresentationMode() {
  ...
  //footer
  let socialMediaSection = document.getElementsByClassName(
...
function MainSearchNightMode() {
  ...
  let searchGifoSection = document.getElementsByClassName(
    "search-gifo-section"
  )[0];
      ...
      if (res.data.length > 0) {
        res.data.forEach((x) => {
          ...
          paragraph.addEventListener("click", () => {
            createGifosGrid(e.target.value);
            console.log(img);
